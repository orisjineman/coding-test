'''
이건 데이원컴퍼니 라이브코딩때 나왔던 문제랑 비슷한 유형임
복습 필요
...
* 셋째 줄부터 맵이 육지인지 바다인지에 대한 정보가 주어진다.
  N개의 줄에 맵의 상태가 북쪽부터 남쪽 순서대로, 각 줄의 데이터는 서쪽부터 동쪽 순서대로 주어진다. 맵의 외곽은 항상 바다로 되어 있다.
    - 0: 육지
    - 1: 바다
...
# 방문한 위치를 저장하기 위한 맵을 생성하여 0으로 초기화 (0: 미방문, 1: 방문)
...
while True:
    ...
    # 회전한 이후 정면에 가보지 않은 칸이 존재하는 경우 이동 (array의 0은 '땅')
    if d[nx][ny] == 0 and array[nx][ny] == 0:
        d[nx][ny] = 1  # 방문 처리하고 처음부터 반복(continue)
    ...
    if turn_time == 4:
        ...
        # 뒤로 갈 수 있다면(땅이라면) 이동하기
        if array[nx][ny] == 0:
            # 후퇴 !!
            x = nx
            y = ny
        # 뒤가 바다로 막혀있는 경우, 종료(break)
        else:
            break
